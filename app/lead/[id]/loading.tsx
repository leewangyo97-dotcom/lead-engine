  ...
  return (
    <Shell current="/">
      <div className="mx-auto max-w-content">
        ...
        <span aria-hidden className="block h-8 w-[256px] animate-pulse rounded-xs bg-sunk" />
        <span aria-hidden className="mt-3 block h-5 w-[320px] animate-pulse rounded-xs bg-sunk" />
        <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {Array.from({ length: 4 }, (_, i) => (
            <span
              key={i}
              aria-hidden
              className="block h-[64px] animate-pulse rounded-sm border border-rule bg-sunk"
            />
          ))}
        </div>
        <span aria-hidden className="mt-8 block h-[160px] w-full max-w-prose animate-pulse rounded-md bg-sunk" />
      </div>
    </Shell>
  );
}
